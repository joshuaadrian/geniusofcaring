...
$layout__arrow = !empty($layout['display_advance_arrow']) ? '<a class="masthead--advance-arrow"><svg  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 19.19 35.88"><polygon points="18.48 25.58 10.09 33.97 10.09 0 9.09 0 9.09 33.97 .71 25.58 0 26.29 9.59 35.88 19.19 26.29 18.48 25.58"/></svg></a>' : '';
...
$content__title = !empty($content['title']) ? '<h1 class="masthead--title serif '.$content__title_size.$content__secondary_title_placement.'" style="color:'.$content__title_color.';text-align:'.$content__title_alignment.';">'.$content__secondary_title.'<span>'.nl2br($content['title']).'</span></h1>' : '';
...
if ( $content__buttons ) : foreach( $content__buttons as $content__button ) :
  ...
  $content__button = $content__button['button'];
    ...
    if ( !empty($content__button) ) :
      ...
      $attributes[] = 'class="masthead--button wp-block-button__link has-green-200-color has-green-200-background-color has-text-color has-background"';
      $attributes   = implode( ' ', $attributes );
      $buttons     .= '<span class="wp-block-button is-style-outline"><a ' . $attributes . '><span>'.$a_title.'</span></a></span>';
...
$content = '<div class="masthead--content" style="'.$content__width.$content__vertical_alignment.$content__horizontal_alignment.'"><div class="masthead--content-inner">'.$content__title.$content__copy.$content__buttons.'</div></div>';
...
$background__vertical_alignment = !empty($background['image_vertical_alignment']) ? 'background-position-x:' . $background['image_vertical_alignment'] . ';' : '';
$background__horizontal_alignment = !empty($background['image_horizontal_alignment']) ? 'background-position-y:' . $background['image_horizontal_alignment'] . ';' : '';
...
$background__video_mp4 = !empty($background['video_mp4']) ? '<div class="masthead--video"><video class="inview fadein" playsinline="playsinline" autoplay="autoplay" muted="muted" loop="loop"><source src="'.$background['video_mp4']['url'].'" type="video/mp4"></video></div>' : '';
