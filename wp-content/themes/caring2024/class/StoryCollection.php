...
class StoryCollection {
    ...
    function __construct($limit=null,$offset=null){
        ...
        if($offset!==null) {
            ...
            $statement = $db->prepare('select user_id FROM goc_users WHERE status = "linked" ORDER BY user_id DESC LIMIT ' . $offset . ',' . $limit);
        } else {
            $statement = $db->prepare('select user_id FROM goc_users WHERE status = "linked" ORDER BY user_id DESC');
        }
        $statement->execute();
